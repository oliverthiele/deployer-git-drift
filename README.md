...
// Exact file paths only — see "Exclude mechanisms outside of Git" below.
...
and the deploy continues. The release is then left without a baseline, and every following
deployment skips `git-drift:check` with a notice — drift tracking never starts, quietly.
Watch for that line in the deploy log: it carries the underlying cause, typically an SSH
host key the server has not accepted for the repository host, a missing deploy key, or an
unreachable remote.
...
- **A whole directory:** add it to `.gitattributes` as `export-ignore`. `git archive` does the prefix matching, so one entry covers everything below it.
- **Individual files:** list them in `git_drift_skip_worktree_paths`.

Note that `git_drift_skip_worktree_paths` matches **exact tracked file paths**. A directory entry such as `Build/` is silently ignored by Git and has no effect, and a path that is not tracked at HEAD makes `git update-index` fail, which aborts the deployment in `git-drift:check`. Keep the list to real file paths and use `export-ignore` for directories.
